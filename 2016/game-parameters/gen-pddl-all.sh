...
for i in $(seq -f "%03g" 1 100); do
	...
	## Only single standing order as goal and input
	# By number of robots
	./to_pddl.py --template=problem.j2 -g game-$i.txt -n ../navgraph-data/navgraph-costs-$i.csv --problem-name-suffix=$i --num-robots=1 --no-rs --durations > ../pddl-production/problem-$i-r1-o1-durations.pddl
	./to_pddl.py --template=problem.j2 -g game-$i.txt -n ../navgraph-data/navgraph-costs-$i.csv --problem-name-suffix=$i --num-robots=2 --no-rs --durations > ../pddl-production/problem-$i-r2-o1-durations.pddl
	./to_pddl.py --template=problem.j2 -g game-$i.txt -n ../navgraph-data/navgraph-costs-$i.csv --problem-name-suffix=$i --num-robots=3 --no-rs --durations > ../pddl-production/problem-$i-r3-o1-durations.pddl
